...
async function geocodeAddress(fullAddress, fallbackAddress, finalFallback) {
    const primary = await tryGeocode(fullAddress);
    if (primary)
        return primary;
    if (fallbackAddress) {
    ...
    throw new AppError_1.AppError(`Could not geocode address: no results found`, 400);
}
...
async function queryOneMirror(mirrorUrl, query) {
    const { data } = await axios_1.default.post(mirrorUrl, new URLSearchParams({ data: query }).toString(), {
        headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
            'User-Agent': 'RealEstateOS/1.0 (development)',
        },
...
async function queryOverpass(query) {
    ...
    for (const mirrorUrl of OVERPASS_MIRRORS) {
        try {
            ...
            const isRetryableElsewhere = err.response?.status === 504 ||
    ...
    throw new AppError_1.AppError('The map data server is busy right now. Please try again in a moment.', 503);
}
async function findNearbyPlaces(latitude, longitude, placeType, radiusMeters = 5000) {
    const tag = placeTagMap[placeType];
    if (!tag)
        throw new AppError_1.AppError(`Unsupported place type: ${placeType}`, 400);
    const [key, value] = tag.split('=');
    const query = `
    [out:json][timeout:25];
    (
